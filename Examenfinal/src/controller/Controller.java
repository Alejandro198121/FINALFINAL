...
public class Controller implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (comando.equals("INICIAR_SESION")) {
            ...
            if (directorioAlmacenes.size() == 6) {
                menuPrincipal.setVisible(false);
                ventanaRegister.setVisible(true);
                ventanaRegister.getPanelRegister().setVisible(true);
            } else {
                mensajero.mostrarMensaje("lo sentimos :( en estos momentos no hay suficientes almacenes");
            }

        } else if (comando.equals("PANEL_INICIAR_SESION")) {
            boolean datosCorrectos = false;
            System.out.println(directorioClientes.getDirectorioClientes().size());
            System.out.println(directorioparejas.size());
            ...
            if (!datosCorrectos) {
                ...
                for (int i = 0; i < directorioClientes.getDirectorioClientes().size(); i++) {
                    if (ventanaLogin.getPanelSesion().getCampoUsuario().getText().equals(directorioClientes.getDirectorioClientes().get(i).getUsuario()) && ventanaLogin.getPanelSesion().getCampoContrasena().getText().equals(directorioClientes.getDirectorioClientes().get(i).getContrasena())) {
                        ...
                        for (int j = 0; j < directorioClientes.getDirectorioClientes().get(i).getParejascliente().size();) {
                            if (ventanaLogin.getPanelSesion().getCampoUsuario().getText().equals(directorioClientes.getDirectorioClientes().get(i).getParejascliente().get(j).getUsuario()) && ventanaLogin.getPanelSesion().getCampoContrasena().getText().equals(directorioClientes.getDirectorioClientes().get(i).getParejascliente().get(j).getContrasena())) {
                                ...
                                this.pareja = directorioparejas.get(j);
                                this.cliente = directorioClientes.getDirectorioClientes().get(i);
                                System.out.println(pareja.toString());
                                System.out.println(directorioparejas.size());
                                System.out.println(pareja.getNombre());
                                datosCorrectos = true;
                                ventanaLogin.setVisible(false);
                ...
                ventanaLogin.getpanelSesion().getCampoUsuario().setText("");
                ventanaLogin.getpanelSesion().getCampoContrasena().setText("");
            }
            ...
            if (menuadmin.getOpciones().getSelectedItem().equals("REGISTRO DE TIENDAS DE MERCADO (ALMACENES)")) {
                ...
                for (Cliente x : directorioClientes.getDirectorioClientes()) {
                    String useraux = x.getUsuario();
                    cupoLiberacion.getClientes().addItem(useraux + " debe: " + x.getDeudatotal());
                }
            ...
            for ( Pareja x : cliente.getParejascliente()) {
                if(cliente.getParejascliente() == null){
                    lugarRepetido = false;
                }else if(pareja.getLugar().equals(x.getLugar())){
                    lugarRepetido = true;
                }
            }
            ...
            } else if (cliente.getParejascliente().size()+1 > cliente.getCantparejascliente()) {
                mensajero.mostrarMensaje("Usted esta intentando registrar mas parejas de las que indico que tiene");
            }else if(lugarRepetido){
                mensajero.mostrarMensaje("Usted no puede registrar dos parejas en el mismo lugar");
            } else if (!lugarRepetido) {
            ...
            if (pareja.getCupo() >= sumaTotal) {
                ...
                cliente.setDeudatotal(cliente.getDeudatotal() + cliente.getDeuda());
